...
//| Step 4.3.1: ATR P70 filter with suppressed ATR logs           |
//+------------------------------------------------------------------+
#property strict
...
void PrintDebug(string strategy, string message)
{
   if(InpPrintDebug)
   {
      Print("[Step4.3.1 ATR ", strategy, "] ", message);
   }
}
...
void PrintAtrDecisionLog(StrategyConfig &cfg, datetime jst_time, string message)
{
   ...
   {
      Print("[Step4.3.1 ATR ", cfg.strategy_name, "] ", message);
   }
}
...
{
   PrintDebug("EA", "Step 4.3.1 ATR log-suppressed initialized.");
   PrintDebug("EA", "Config-managed 28 strategy EA with Global H1 ATR P70 and suppressed ATR logs.");
