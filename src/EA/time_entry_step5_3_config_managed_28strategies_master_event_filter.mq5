...
//| Step 5.3: Master-list aligned event filter           |
//+------------------------------------------------------------------+
#property strict
...
void PrintDebug(string strategy, string message)
{
   if(InpPrintDebug)
   {
      Print("[Step5.3 Master ", strategy, "] ", message);
   }
}
...
void PrintAtrDecisionLog(StrategyConfig &cfg, datetime jst_time, string message)
{
   ...
   {
      Print("[Step5.3 Master ", cfg.strategy_name, "] ", message);
   }
}
...
void PrintEventDecisionLog(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   ...
   {
      int key = DateKey(jst_time);
      Print("[Step5.3 Master ", cfg.strategy_name, "] EVENT REJECT. Symbol=", cfg.symbol, ", Event=", event_name, ", Date=", IntegerToString(key));
   }
}
