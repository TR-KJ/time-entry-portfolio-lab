...
#
# v1.2 Add Aussie Logic - pip_size / spread_pips fixed
#
# ベース：
# - v1.1.1 AJ Event Fix
# - クロス円16ロジック
...
#
# 修正：
# - JPYペア / 非JPYペアのpip_size対応
# - spreadを価格値ではなくpips指定へ変更
# - SL/TP/損益計算をpip_sizeベースへ変更
# ==========================================
...
# AUD_CPI_DATES
#
# 2026年分も追加済みの状態にしてください。
# ==========================================


# ==========================================
# 1-B. pip_size / spread定義
# ==========================================

PIP_SIZE_JPY = 0.01
PIP_SIZE_NONJPY = 0.0001
...
# --- EJ ---
run_strategy('EJ', '1_EJ_Log1', t_ej, o_ej, h_ej, l_ej, idx_ej, dates_ej, True, PIP_SIZE_JPY, SPREADS_PIPS['EJ'], lambda ds, dt: is_year_end(dt) or dt.month == 2 or dt.day == 1 or ds in cpi_wednesdays, [0, 2], 13, 55, 4, 55, 1, 70, 250)

run_strategy('EJ', '2_EJ_NightBlitz_20', t_ej, o_ej, h_ej, l_ej, idx_ej, dates_ej, True, PIP_SIZE_JPY, SPREADS_PIPS['EJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_5_EJ, [0, 2], 20, 56, 4, 45, 1, 45, 70)

run_strategy('EJ', '3_EJ_NightBlitz_21', t_ej, o_ej, h_ej, l_ej, idx_ej, dates_ej, True, PIP_SIZE_JPY, SPREADS_PIPS['EJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_5_EJ, [0, 2], 21, 56, 5, 27, 1, 75, 70)

# --- GJ ---
run_strategy('GJ', '4_GJ_Port_Log1', t_gj, o_gj, h_gj, l_gj, idx_gj, dates_gj, True, PIP_SIZE_JPY, SPREADS_PIPS['GJ'], lambda ds, dt: is_year_end(dt) or dt.month == 12 or dt.day in [1, 2, 29, 30, 31], [1, 2], 0, 0, 8, 55, 0, 130, 90)

run_strategy('GJ', '5_GJ_Port_Log2', t_gj, o_gj, h_gj, l_gj, idx_gj, dates_gj, False, PIP_SIZE_JPY, SPREADS_PIPS['GJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_5_GJ or dt.day in [18, 19, 27], [1, 3, 4], 9, 55, 23, 55, 0, 90, 999)

run_strategy('GJ', '6_GJ_Old_Mon', t_gj, o_gj, h_gj, l_gj, idx_gj, dates_gj, True, PIP_SIZE_JPY, SPREADS_PIPS['GJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_5_GJ or dt.month in [1, 2], [0], 15, 45, 22, 50, 0, 50, 210)

run_strategy('GJ', '7_GJ_Mon_Blitz', t_gj, o_gj, h_gj, l_gj, idx_gj, dates_gj, True, PIP_SIZE_JPY, SPREADS_PIPS['GJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_5_GJ, [0], 18, 2, 23, 2, 0, 130, 250)

# --- AJ ---
run_strategy('AJ', '8_AJ_Core1', t_aj, o_aj, h_aj, l_aj, idx_aj, dates_aj, True, PIP_SIZE_JPY, SPREADS_PIPS['AJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_6_AJ, [0], 8, 1, 22, 46, 0, 70, 110)

run_strategy('AJ', '9_AJ_Core2', t_aj, o_aj, h_aj, l_aj, idx_aj, dates_aj, False, PIP_SIZE_JPY, SPREADS_PIPS['AJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_6_AJ or dt.month in [6, 9] or dt.day in [1, 20] or dt.day >= 26, [3], 17, 14, 1, 14, 1, 30, 80)

run_strategy('AJ', '10_AJ_SatA', t_aj, o_aj, h_aj, l_aj, idx_aj, dates_aj, False, PIP_SIZE_JPY, SPREADS_PIPS['AJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_6_AJ, [4], 10, 58, 13, 51, 0, 50, 25)

run_strategy('AJ', '11_AJ_SatB', t_aj, o_aj, h_aj, l_aj, idx_aj, dates_aj, False, PIP_SIZE_JPY, SPREADS_PIPS['AJ'], lambda ds, dt: is_year_end(dt) or ds in EVENTS_6_AJ, [4], 18, 57, 1, 43, 1, 55, 95)

# --- UJ ---
f_uj_4 = lambda ds, dt: is_year_end(dt) or ds in EVENTS_4

run_strategy('UJ', '12_UJ_Short_Core', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, False, PIP_SIZE_JPY, SPREADS_PIPS['UJ'], f_uj_4, [], 9, 55, 14, 56, 0, 20, 50, True, 'short_core_calendar_end')

run_strategy('UJ', '13_UJ_Fix_MidWeek', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, True, PIP_SIZE_JPY, SPREADS_PIPS['UJ'], f_uj_4, [], 18, 4, 22, 3, 0, 95, 95, True, '25_onwards_wed_thu')

run_strategy('UJ', '14_UJ_Sat_3rd', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, False, PIP_SIZE_JPY, SPREADS_PIPS['UJ'], f_uj_4, [], 20, 1, 3, 8, 1, 45, 70, True, '3rd')

run_strategy('UJ', '15_UJ_Sat_Aug', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, False, PIP_SIZE_JPY, SPREADS_PIPS['UJ'], f_uj_4, [], 19, 0, 23, 30, 0, 20, 35, True, 'aug_1_10')

run_strategy('UJ', '16_UJ_T10A', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, True, PIP_SIZE_JPY, SPREADS_PIPS['UJ'], lambda ds, dt: is_year_end(dt) or ds in BOJ_DATES, [], 2, 58, 9, 50, 0, 45, 110, True, '10th_not_wed')
...
run_strategy('EA', '17_EA_1B_Wed_Short', t_ea, o_ea, h_ea, l_ea, idx_ea, dates_ea, False, PIP_SIZE_NONJPY, SPREADS_PIPS['EA'], lambda ds, dt: filter_ea_rev4(ds, dt) or dt.month == 8, [2], 9, 59, 20, 58, 0, 70, 175)

run_strategy('EA', '18_EA_2_MonWed_Short', t_ea, o_ea, h_ea, l_ea, idx_ea, dates_ea, False, PIP_SIZE_NONJPY, SPREADS_PIPS['EA'], lambda ds, dt: filter_ea_rev4(ds, dt) or dt.month in [1, 8], [0, 1, 2], 9, 59, 5, 26, 1, 90, 180)

run_strategy('EA', '19_EA_3_WedThu_Long', t_ea, o_ea, h_ea, l_ea, idx_ea, dates_ea, True, PIP_SIZE_NONJPY, SPREADS_PIPS['EA'], filter_ea_rev4, [2, 3], 20, 56, 10, 0, 1, 90, 999)

run_strategy('EA', '20_EA_1A_MonTue_Short', t_ea, o_ea, h_ea, l_ea, idx_ea, dates_ea, False, PIP_SIZE_NONJPY, SPREADS_PIPS['EA'], lambda ds, dt: filter_ea_rev4(ds, dt) or dt.month == 8, [0, 1], 10, 1, 16, 0, 0, 50, 125)
...
run_strategy('GA', '21_GA_B_3', t_ga, o_ga, h_ga, l_ga, idx_ga, dates_ga, True, PIP_SIZE_NONJPY, SPREADS_PIPS['GA'], filter_ga, [0], 21, 2, 10, 0, 1, 220, 100)

run_strategy('GA', '22_GA_C_2', t_ga, o_ga, h_ga, l_ga, idx_ga, dates_ga, True, PIP_SIZE_NONJPY, SPREADS_PIPS['GA'], filter_ga, [3], 16, 56, 1, 15, 1, 70, 80)

run_strategy('GA', '23_GA_F_2', t_ga, o_ga, h_ga, l_ga, idx_ga, dates_ga, False, PIP_SIZE_NONJPY, SPREADS_PIPS['GA'], filter_ga, [4], 19, 42, 22, 45, 0, 90, 200)

run_strategy('GA', '24_GA_D_1', t_ga, o_ga, h_ga, l_ga, idx_ga, dates_ga, True, PIP_SIZE_NONJPY, SPREADS_PIPS['GA'], filter_ga, [4], 22, 44, 3, 8, 1, 90, 200)
...
run_strategy('AU', '25_AU_China_Demand', t_au, o_au, h_au, l_au, idx_au, dates_au, True, PIP_SIZE_NONJPY, SPREADS_PIPS['AU'], filter_au_china, [], 10, 0, 15, 50, 0, 40, 40, False, None, au_china_rule)

run_strategy('AJ', '26_AJ_China_Demand', t_aj, o_aj, h_aj, l_aj, idx_aj, dates_aj, True, PIP_SIZE_JPY, SPREADS_PIPS['AJ'], filter_aj_china, [], 10, 0, 15, 50, 0, 45, 80, False, None, aj_china_rule)

run_strategy('EA', '27_EA_China_Demand', t_ea, o_ea, h_ea, l_ea, idx_ea, dates_ea, False, PIP_SIZE_NONJPY, SPREADS_PIPS['EA'], filter_ea_china, [], 10, 0, 15, 50, 0, 60, 60, False, None, ea_china_rule)

run_strategy('GA', '28_GA_China_Demand', t_ga, o_ga, h_ga, l_ga, idx_ga, dates_ga, False, PIP_SIZE_NONJPY, SPREADS_PIPS['GA'], filter_ga_china, [], 10, 0, 16, 10, 0, 75, 70, False, None, ga_china_rule)
