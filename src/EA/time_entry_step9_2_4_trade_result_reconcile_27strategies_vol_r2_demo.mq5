//+------------------------------------------------------------------+
//| Phase 5: 27 enabled strategies, R2 allocation, OANDA DEMO ONLY |
//| Time Entry Portfolio Lab                                         |
//| Step 9.2.4: Step 9.2.3 plus trade-result reconciliation          |
//+------------------------------------------------------------------+
#property strict
// Phase5 demo-only inputs: template is deliberately unable to trade.
input bool InpPhase5Approved = false;
...
#include "phase5_demo/step921_demo_dependency.mqh"
...
#include "phase5_demo/vol_r2_runtime.mqh"
...
{
   if(!P5Guard()) return 0;
   p5_equity=AccountInfoDouble(ACCOUNT_EQUITY);
   p5_balance=AccountInfoDouble(ACCOUNT_BALANCE);
   p5_week_action="REUSED";
   MqlDateTime dt;
   TimeToStruct(jst_time, dt);
   ...
   p5_week_action="CREATED";
   double base_amount = p5_equity;

   if(base_amount <= 0)
   {
      base_amount = p5_balance;
   }

   if(base_amount > 0)
   {
      if(GlobalVariableSet(gv_name, base_amount)==0) return 0;
   }
...
//+------------------------------------------------------------------+
double GetStrategyLot(StrategyConfig &cfg,datetime jst_time) { return P5GetLot(cfg,jst_time); }
...
void ClosePositionsByConfig(StrategyConfig &cfg)
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      ResetLastError();
      if(!P5Guard()) return;
      P5Emit(cfg,"EXIT_ATTEMPT","PositionId="+TicketToText(ticket));
      ...
      P5Emit(cfg,"EXIT_RESULT","Retcode="+IntegerToString(pending.retcode)+"|OrderTicket="+TicketToText(pending.result_order)+"|DealTicket="+TicketToText(pending.result_deal)+"|PositionId="+TicketToText(pending.position_identifier));
...
void PrintTradeResultDiagnostic(StrategyConfig &cfg,
                                ...
                                int last_error)
{
   P5Emit(cfg,"ORDER_RESULT","Retcode="+IntegerToString(retcode)+"|OrderTicket="+TicketToText(result_order)+"|DealTicket="+TicketToText(result_deal));
   if(!InpPrintTradeResultDiagnostics) return;
...
bool SendBuyOrder(StrategyConfig &cfg, datetime jst_time)
{
   ...
   ResetLastError();
   if(!P5Guard()) return false;
   if(!IsEntryTime(cfg,P5Now()) || P5Midnight(P5Now())!=P5Midnight(jst_time)) { P5Decision(cfg,"WINDOW_EXPIRED_DURING_SIZING"); return false; }
   P5Emit(cfg,"ORDER_ATTEMPT","OrderAttempt=true|Direction=BUY");
...
bool SendSellOrder(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(!P5Guard()) return false;
   if(!IsEntryTime(cfg,P5Now()) || P5Midnight(P5Now())!=P5Midnight(jst_time)) { P5Decision(cfg,"WINDOW_EXPIRED_DURING_SIZING"); return false; }
   P5Emit(cfg,"ORDER_ATTEMPT","OrderAttempt=true|Direction=SELL");
...
void TryEntry(StrategyConfig &cfg, datetime jst_time)
{
   ...
   P5Candidate(cfg,jst_time);
   if(!EnsureSymbolReady(cfg.symbol,cfg.strategy_name)){ P5Decision(cfg,"SYMBOL_NOT_READY"); return; }
   if(!PassWeekendMarketClosedGuard(cfg, jst_time))
   {
      P5Decision(cfg,"WEEKEND_GUARD");
      return;
   }
   if(InpEmergencyStop)
   {
      P5Decision(cfg,"EMERGENCY_STOP");
      PrintEmergencyStopSkip(cfg, jst_time);
      return;
   }
   if(!PassEntryFilters(cfg, jst_time))
   {
      P5Decision(cfg,"ENTRY_FILTER_REJECTED");
      PrintSkip(cfg.strategy_name, "Skip entry: entry filter rejected.");
      return;
   }
   if(AlreadyEnteredToday(cfg, jst_time)){ P5Decision(cfg,"ALREADY_ENTERED_TODAY"); PrintSkipOncePerDay(cfg, jst_time, "already_entered_today", "Skip entry: already entered today."); return; }
   int strategy_index = FindStrategyIndexByMagicAndSymbol(cfg.magic, cfg.symbol);
   if(HasPendingEntryReconciliation(strategy_index)){ P5Decision(cfg,"ENTRY_RECONCILIATION_PENDING"); PrintSkipOncePerDay(cfg, jst_time, "entry_reconciliation_pending", "Skip entry: entry reconciliation pending."); return; }
   if(HasOpenPosition(cfg.symbol, cfg.magic)){ P5Decision(cfg,"POSITION_ALREADY_EXISTS"); PrintSkipOncePerDay(cfg, jst_time, "position_already_exists", "Skip entry: position already exists."); return; }
   P5Decision(cfg,"ELIGIBLE");
