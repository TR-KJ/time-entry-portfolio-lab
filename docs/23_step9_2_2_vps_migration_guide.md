...
VPSへは、MacまたはDELLのどちらからでも接続できる。

### Macから接続する場合

Mac App StoreからMicrosoft公式の`Windows App`をインストールする。

操作:

1. `Windows App`を開く
2. `＋`または`Add PC`を選択する
3. `PC Name`へVPSのIPアドレスを入力する
4. VPSのWindowsユーザー名を登録する
5. VPSのパスワードを入力する
6. 接続先を開く
7. 証明書の確認画面が表示された場合は、接続先IPを再確認する
8. 問題なければ接続する
...
2. `mstsc`と入力する
3. `Enter`を押す
4. `コンピューター`欄へVPSのIPアドレスを入力する
5. `接続`を押す
6. VPSのWindowsユーザー名とパスワードを入力する
7. 問題なければ接続する

接続後、VPSのWindowsデスクトップが表示されれば接続成功。

初回のEA・SETファイル転送は、必要なファイルが保存されているDELLから接続する方が分かりやすい。

構築後の日常的な監視は、Macから接続しても問題ない。
