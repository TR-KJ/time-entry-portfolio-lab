// Dedicated copy; changes documented in source manifest. Never overwrite live.
//+------------------------------------------------------------------+
//| time_entry_step9_2_1_event_candidate_c_overlap_fix_28strategies.mq5                 |
//| Time Entry Portfolio Lab                                         |
//| Step 9.2.1: Event Candidate C overlap fix for 28 strategies         |
//+------------------------------------------------------------------+
#property strict
...
input int    InpLotMode = 1;
input bool   InpWeeklyBaseUseEquity = true;
input double InpRiskPercentPerTrade = 0.90;
input double InpMaxAutoLot = 1.00;
input bool   InpAllowMinLotWhenBelowMinimum = false;
...
input bool   InpTestModeIgnoreEntryWeekday = false;
input bool   InpTestModeIgnoreExitWeekday = false;
...
input bool            InpUseGlobalAtrP70Filter = false;
...
input bool            InpUseEventCandidateC = true;

// Event Candidate C time-window settings, JST based.
input int             InpFomcEventHourJST = 3;
input int             InpFomcEventMinuteJST = 0;
input int             InpFomcPreMinutes = 180;
input int             InpFomcPostMinutes = 180;
input int             InpBoeEventHourJST = 21;
input int             InpBoeEventMinuteJST = 0;
input int             InpBoePreMinutes = 120;
input int             InpBoePostMinutes = 120;
input int             InpEcbEventHourJST = 21;
input int             InpEcbEventMinuteJST = 15;
input int             InpEcbPreMinutes = 120;
input int             InpEcbPostMinutes = 120;
input int             InpRbaEventHourJST = 13;
input int             InpRbaEventMinuteJST = 30;
input int             InpRbaPreMinutes = 120;
input int             InpRbaPostMinutes = 120;
input int             InpAuCpiEventHourJST = 10;
input int             InpAuCpiEventMinuteJST = 30;
input int             InpAuCpiPreMinutes = 120;
input int             InpAuCpiPostMinutes = 120;
...
input bool InpEnable_22_GA_C2 = false;
...
//+------------------------------------------------------------------+
// Audit scope: 2026-08-28 and later only. Earlier verified dates remain identical to main.
// Dates are JST calendar dates. Future FOMC dates are the JST statement dates
// (the next day from the U.S. meeting date).
...
int US_CPI_2026_DATES[] = {20260114,20260211,20260311,20260415,20260513,20260610,20260714,20260812,20260911,20261014,20261110,20261210};
...
int US_CPI_WED_2026_DATES[] = {20260114,20260211,20260311,20260415,20260513,20260610,20260714,20260812,20260909,20261014,20261111,20261209};
...
datetime GetJstTime() { return P5Now(); }
...
string AtrLogGlobalVariableName(StrategyConfig &cfg,datetime jst_time) { return P5GV("A",IntegerToString(cfg.magic)+"_"+IntegerToString(DateKey(jst_time))); }
...
//+------------------------------------------------------------------+
string EventLogGlobalVariableName(StrategyConfig &cfg,datetime jst_time,string event_name) { return P5GV("L",IntegerToString(cfg.magic)+"_"+IntegerToString(DateKey(jst_time))+"_"+IntegerToString((long)P5Hash(event_name))); }
...
string WeeklyBaseGlobalVariableName(int week_key) { return P5GV("W",IntegerToString(week_key)); }
...
string EntryGlobalVariableName(StrategyConfig &cfg,int date_key) { return P5GV("E",IntegerToString(cfg.magic)+"_"+IntegerToString(date_key)); }
...
string NormalizeCandidateCEventName(string event_name)
{
...
bool IsCandidateCForceDateAllDayStrategy(StrategyConfig &cfg)
{
   if(cfg.strategy_code == "2_EJ_NightBlitz_20") return true;
   if(cfg.strategy_code == "3_EJ_NightBlitz_21") return true;
   if(cfg.strategy_code == "7_GJ_Mon_Blitz") return true;
   if(cfg.strategy_code == "10_AJ_SatA") return true;
   if(cfg.strategy_code == "11_AJ_SatB") return true;
   return false;
}
...
int GetCandidateCEventPolicy(StrategyConfig &cfg, string event_name)
{
   ...
   if(IsCandidateCForceDateAllDayStrategy(cfg))
   {
      return EVENT_POLICY_DATE_ALL_DAY;
   }
   ...
   {
      return EVENT_POLICY_POSITION_OVERLAP;
   }

   if(normalized_event == "RBA")
   {
      return EVENT_POLICY_POSITION_OVERLAP;
   }

   if(normalized_event == "ECB")
   {
      return EVENT_POLICY_POSITION_OVERLAP;
   }

   if(normalized_event == "BOE")
   {
      return EVENT_POLICY_POSITION_OVERLAP;
   }

   if(normalized_event == "AU_CPI")
   {
      return EVENT_POLICY_POSITION_OVERLAP;
   }
...
int EventDateKeyFromJstTime(string event_name, datetime jst_time)
{
   string normalized_event = NormalizeCandidateCEventName(event_name);

   if(event_name == "FOMC_PREV" || normalized_event == "FOMC")
   {
      MqlDateTime base_dt;
      TimeToStruct(jst_time, base_dt);
      int date_key = DateKey(jst_time);

      if(IsDateInList(date_key, FOMC_2026_DATES))
      {
         return date_key;
      }

      datetime next_day = jst_time + 86400;
      int next_key = DateKey(next_day);

      if(IsDateInList(next_key, FOMC_2026_DATES))
      {
         return next_key;
      }
   }

   return DateKey(jst_time);
}

datetime BuildJstDateTimeFromDateKey(int date_key, int hour, int minute)
{
   MqlDateTime dt;
   dt.year = date_key / 10000;
   dt.mon = (date_key / 100) % 100;
   dt.day = date_key % 100;
   dt.hour = hour;
   dt.min = minute;
   dt.sec = 0;
...
bool GetCandidateCEventTimeAndWindow(string event_name, datetime jst_time, datetime &event_time_jst, int &pre_minutes, int &post_minutes)
{
   ...
   if(normalized_event == "BOE")
   {
      event_time_jst = BuildJstDateTimeFromDateKey(event_date_key, InpBoeEventHourJST, InpBoeEventMinuteJST);
      pre_minutes = InpBoePreMinutes;
      post_minutes = InpBoePostMinutes;
      return true;
   }
   ...
   if(normalized_event == "RBA")
   {
      event_time_jst = BuildJstDateTimeFromDateKey(event_date_key, InpRbaEventHourJST, InpRbaEventMinuteJST);
      pre_minutes = InpRbaPreMinutes;
      post_minutes = InpRbaPostMinutes;
      return true;
   }

   if(normalized_event == "AU_CPI")
   {
      event_time_jst = BuildJstDateTimeFromDateKey(event_date_key, InpAuCpiEventHourJST, InpAuCpiEventMinuteJST);
      pre_minutes = InpAuCpiPreMinutes;
      post_minutes = InpAuCpiPostMinutes;
      return true;
   }
...
datetime BuildPlannedEntryTime(StrategyConfig &cfg, datetime jst_time)
{
   MqlDateTime dt;
   TimeToStruct(jst_time, dt);
...
datetime BuildPlannedExitTime(StrategyConfig &cfg, datetime jst_time, datetime entry_time)
{
   MqlDateTime dt;
   TimeToStruct(jst_time, dt);
...
bool DoesPlannedPositionOverlapEventWindow(StrategyConfig &cfg, datetime jst_time, string event_name, datetime &event_time_jst, int &pre_minutes, int &post_minutes)
{
   if(!GetCandidateCEventTimeAndWindow(event_name, jst_time, event_time_jst, pre_minutes, post_minutes))
   {
      return false;
   }

   datetime entry_time = BuildPlannedEntryTime(cfg, jst_time);
   datetime exit_time = BuildPlannedExitTime(cfg, jst_time, entry_time);
...
bool IsCandidateCEventDateKey(string event_name, int date_key)
{
   string normalized_event = NormalizeCandidateCEventName(event_name);

   if(normalized_event == "FOMC")
   {
      return IsDateInList(date_key, FOMC_2026_DATES);
   }

   if(normalized_event == "RBA")
   {
      return IsDateInList(date_key, RBA_2026_DATES);
   }

   if(normalized_event == "ECB")
   {
      return IsDateInList(date_key, ECB_2026_DATES);
   }

   if(normalized_event == "BOE")
   {
      return IsDateInList(date_key, BOE_2026_DATES);
   }

   if(normalized_event == "AU_CPI")
   {
      return IsDateInList(date_key, AU_CPI_2026_DATES);
   }

   return false;
}

bool IsCandidateCStrategyTargetForEvent(StrategyConfig &cfg, string event_name)
{
   string normalized_event = NormalizeCandidateCEventName(event_name);
   ...
   if(normalized_event == "FOMC")
   {
      if(cfg.strategy_code == "2_EJ_NightBlitz_20") return true;
      if(cfg.strategy_code == "3_EJ_NightBlitz_21") return true;
      if(cfg.strategy_code == "5_GJ_Log2") return true;
      if(cfg.strategy_code == "6_GJ_Old_Mon") return true;
      if(cfg.strategy_code == "7_GJ_Mon_Blitz") return true;
      if(cfg.strategy_code == "8_AJ_Core1") return true;
      if(cfg.strategy_code == "9_AJ_Core2") return true;
      if(cfg.strategy_code == "10_AJ_SatA") return true;
      if(cfg.strategy_code == "11_AJ_SatB") return true;
      if(cfg.strategy_code == "12_UJ_Short_Core") return true;
      if(cfg.strategy_code == "13_UJ_Fix_MidWeek") return true;
      if(cfg.strategy_code == "14_UJ_Sat_3rd") return true;
      if(cfg.strategy_code == "15_UJ_Sat_Aug") return true;
      if(cfg.strategy_code == "17_EA_1B") return true;
      if(cfg.strategy_code == "18_EA_2") return true;
      if(cfg.strategy_code == "19_EA_3") return true;
      if(cfg.strategy_code == "20_EA_1A") return true;
      if(cfg.strategy_code == "21_GA_B3") return true;
      if(cfg.strategy_code == "22_GA_C2") return true;
      if(cfg.strategy_code == "23_GA_F2") return true;
      if(cfg.strategy_code == "24_GA_D1") return true;
      if(cfg.strategy_code == "25_AU_China_Demand") return true;
      if(cfg.strategy_code == "27_EA_China_Demand") return true;
      if(cfg.strategy_code == "28_GA_China_Demand") return true;
      return false;
   }

   if(normalized_event == "RBA" || normalized_event == "AU_CPI")
   {
      if(cfg.strategy_code == "8_AJ_Core1") return true;
      if(cfg.strategy_code == "9_AJ_Core2") return true;
      if(cfg.strategy_code == "10_AJ_SatA") return true;
      ...
      if(cfg.strategy_code == "24_GA_D1") return true;
      if(cfg.strategy_code == "25_AU_China_Demand") return true;
      if(cfg.strategy_code == "26_AJ_China_Demand") return true;
      if(cfg.strategy_code == "27_EA_China_Demand") return true;
      if(cfg.strategy_code == "28_GA_China_Demand") return true;
      return false;
   }
...
bool IsCandidateCPositionOverlapEventApplicable(StrategyConfig &cfg, string event_name)
{
   if(!IsCandidateCStrategyTargetForEvent(cfg, event_name))
   {
      return false;
   }

   if(IsCandidateCPositionOverlapOverride(cfg, event_name))
   {
      return true;
   }

   int policy = GetCandidateCEventPolicy(cfg, event_name);

   if(policy == EVENT_POLICY_POSITION_OVERLAP)
   {
      return true;
   }
...
bool GetCandidateCEventTimeAndWindowByDateKey(string event_name, int event_date_key, datetime &event_time_jst, int &pre_minutes, int &post_minutes)
{
   string normalized_event = NormalizeCandidateCEventName(event_name);
...
bool DoesPlannedPositionOverlapEventWindowByDateKey(StrategyConfig &cfg, datetime jst_time, string event_name, int event_date_key, datetime &event_time_jst, int &pre_minutes, int &post_minutes)
{
   if(!GetCandidateCEventTimeAndWindowByDateKey(event_name, event_date_key, event_time_jst, pre_minutes, post_minutes))
   {
      return false;
   }
...
bool RejectCandidateCPositionOverlapEventIfMatched(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   if(!IsCandidateCPositionOverlapEventApplicable(cfg, event_name))
   {
      return false;
   }
   ...
   if(IsCandidateCEventDateKey(event_name, date_key))
   {
      if(DoesPlannedPositionOverlapEventWindowByDateKey(cfg, jst_time, event_name, date_key, event_time_jst, pre_minutes, post_minutes))
      {
   ...
   if(next_key != date_key && IsCandidateCEventDateKey(event_name, next_key))
   {
      if(DoesPlannedPositionOverlapEventWindowByDateKey(cfg, jst_time, event_name, next_key, event_time_jst, pre_minutes, post_minutes))
      {
         PrintCandidateCOverlapStopLog(cfg, jst_time, event_name, event_time_jst, pre_minutes, post_minutes);
         return true;
      }
...
void PrintCandidateCDateStopLog(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   PrintEventDecisionLog(cfg, jst_time, "CANDIDATE_C_DATE_" + NormalizeCandidateCEventName(event_name));
}

void PrintCandidateCOverlapStopLog(StrategyConfig &cfg, datetime jst_time, string event_name, datetime event_time_jst, int pre_minutes, int post_minutes)
{
   string normalized_event = NormalizeCandidateCEventName(event_name);
   string event_log_name = "CANDIDATE_C_OVERLAP_" + normalized_event;

   if(ShouldPrintEventDecisionLog(cfg, jst_time, event_log_name))
   {
      datetime entry_time = BuildPlannedEntryTime(cfg, jst_time);
      datetime exit_time = BuildPlannedExitTime(cfg, jst_time, entry_time);
      Print(
         "[Step9 EventCandidateC ",
         cfg.strategy_name,
         "] EVENT OVERLAP STOP. Symbol=",
         cfg.symbol,
         ", Event=",
         normalized_event,
         ", Entry=",
         DateTimeText(entry_time),
         ", Exit=",
         DateTimeText(exit_time),
         ", EventTime=",
         DateTimeText(event_time_jst),
         ", WindowPre=",
         IntegerToString(pre_minutes),
         ", WindowPost=",
         IntegerToString(post_minutes)
      );
   }
}

bool IsCandidateCEventRejected(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   int policy = GetCandidateCEventPolicy(cfg, event_name);

   if(policy == EVENT_POLICY_NONE)
   {
      return false;
   }
   ...
   {
      PrintCandidateCDateStopLog(cfg, jst_time, event_name);
      return true;
   }
   ...
   if(policy == EVENT_POLICY_POSITION_OVERLAP)
   {
      ...
      if(DoesPlannedPositionOverlapEventWindow(cfg, jst_time, event_name, event_time_jst, pre_minutes, post_minutes))
      {
         PrintCandidateCOverlapStopLog(cfg, jst_time, event_name, event_time_jst, pre_minutes, post_minutes);
         return true;
      }
...
bool PassEventCandidateCFilter(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(cfg.strategy_code == "2_EJ_NightBlitz_20" || cfg.strategy_code == "3_EJ_NightBlitz_21")
   {
      ...
      {
         if(IsCandidateCEventRejected(cfg, jst_time, event_name)) return false;
      }
...
bool PassPythonCalendarEventFilter(StrategyConfig &cfg, datetime jst_time)
{
   ...
   {
      return PassEventCandidateCFilter(cfg, jst_time);
   }
...
//+------------------------------------------------------------------+
//| Step 8.3.1 skip-log suppression helpers                          |
//+------------------------------------------------------------------+
string printed_skip_log_keys[];
int printed_skip_log_count = 0;

bool WasSkipLogPrintedRuntime(string log_key)
{
   for(int i = 0; i < printed_skip_log_count; i++)
   {
      if(printed_skip_log_keys[i] == log_key)
      {
         return true;
      }
   }
...
void MarkSkipLogPrintedRuntime(string log_key)
{
   int size = ArraySize(printed_skip_log_keys);
   ArrayResize(printed_skip_log_keys, size + 1);
   printed_skip_log_keys[size] = log_key;
   printed_skip_log_count = size + 1;
}

void PrintSkipOncePerDay(StrategyConfig &cfg, datetime jst_time, string reason, string message)
{
   if(!InpSuppressSkipLogsOncePerDay)
   {
      PrintSkip(cfg.strategy_name, message);
      return;
   }

   int key = DateKey(jst_time);

   string log_key = cfg.strategy_code + "_" +
                    cfg.symbol + "_" +
                    IntegerToString(cfg.magic) + "_" +
                    IntegerToString(key) + "_" +
                    reason;

   if(WasSkipLogPrintedRuntime(log_key))
   {
      return;
   }

   MarkSkipLogPrintedRuntime(log_key);
   PrintSkip(cfg.strategy_name, message);
}
...
{
   PrintDebug("EA", "Step 9.1 Weekend Guard EA initialized.");
   PrintDebug("EA", "Config-managed 28 strategy EA with weekend guard, emergency stop and lot mode check.");
   ...
   PrintDebug("EA", "UseWeekendMarketClosedGuard=" + BoolText(InpUseWeekendMarketClosedGuard));
   PrintDebug("EA", "PrintWeekendGuardLogs=" + BoolText(InpPrintWeekendGuardLogs));
   PrintDebug("EA", "SuppressWeekendGuardLogsOncePerDay=" + BoolText(InpSuppressWeekendGuardLogsOncePerDay));
   ...
   PrintDebug("EA", "UseEventCandidateC=" + BoolText(InpUseEventCandidateC));
   PrintDebug("EA", "FOMC Event Time JST=" + IntegerToString(InpFomcEventHourJST) + ":" + IntegerToString(InpFomcEventMinuteJST));
   PrintDebug("EA", "FOMC Window Minutes=" + IntegerToString(InpFomcPreMinutes) + "/" + IntegerToString(InpFomcPostMinutes));
...
{
   EventKillTimer();
   PrintDebug("EA", "Step 9.1 Weekend Guard EA deinitialized. Reason=" + IntegerToString(reason));
}
