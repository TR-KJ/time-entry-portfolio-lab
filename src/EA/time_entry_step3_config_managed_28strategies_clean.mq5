...
//| Step 3.2: Clean config-managed 28 strategy EA                    |
//|                                                                  |
//| Purpose:                                                         |
//| - Keep Step 2J.2 behavior                                        |
//| - Clean function structure                                       |
//| - Prepare for future filters                                     |
//|   - Global H1 ATR P70                                            |
//|   - Event stop                                                   |
//|   - Year-end stop                                                |
//|   - Weekly compounding lot                                       |
//+------------------------------------------------------------------+
#property strict
...
void PrintDebug(string strategy, string message)
{
   if(InpPrintDebug)
   {
      Print("[Step3 Clean ", strategy, "] ", message);
   }
}
...
{
   string name = "TE_STEP3_" + cfg.strategy_code + "_" + cfg.symbol + "_" + IntegerToString(cfg.magic) + "_" + IntegerToString(date_key);
...
bool IsChinaAUDemandActiveDate(datetime jst_time, string strategy_name)
{
   ...
   if(!IsWeekday(dt))
   {
      PrintRuleReject(strategy_name, "Date rule reject: not weekday. JST=" + DateTimeText(jst_time));
      return false;
   }

   bool is_first_window = false;
   bool is_second_window = false;

   if(dt.day >= 9 && dt.day <= 15)
   {
...
bool IsAJCore2ActiveDate(datetime jst_time, string strategy_name)
{
   ...
   if(!IsThursday(dt))
   {
      PrintRuleReject(strategy_name, "Date rule reject: not Thursday. JST=" + DateTimeText(jst_time));
      return false;
   }

   if(dt.mon == 6)
   {
      PrintRuleReject(strategy_name, "Date rule reject: June stop. JST=" + DateTimeText(jst_time));
      return false;
   }

   if(dt.mon == 9)
   {
      PrintRuleReject(strategy_name, "Date rule reject: September stop. JST=" + DateTimeText(jst_time));
      return false;
   }

   if(dt.day == 1)
   {
      PrintRuleReject(strategy_name, "Date rule reject: 1st stop. JST=" + DateTimeText(jst_time));
      return false;
   }

   if(dt.day == 20)
   {
      PrintRuleReject(strategy_name, "Date rule reject: 20th stop. JST=" + DateTimeText(jst_time));
      return false;
   }

   if(dt.day >= 26)
   {
      PrintRuleReject(strategy_name, "Date rule reject: after 26th stop. JST=" + DateTimeText(jst_time));
      return false;
   }
...
bool IsStrategyActiveDate(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(cfg.special_rule == RULE_NONE)
   {
      return IsAllowedWeekday(cfg, dt.day_of_week);
   }
   ...
   if(cfg.special_rule == RULE_CHINA_AU_DEMAND)
   {
      return IsChinaAUDemandActiveDate(jst_time, cfg.strategy_name);
   }

   if(cfg.special_rule == RULE_CHINA_9_15)
   {
      return IsChina915ActiveDate(jst_time, cfg.strategy_name);
   }

   if(cfg.special_rule == RULE_AJ_CORE2)
   {
      return IsAJCore2ActiveDate(jst_time, cfg.strategy_name);
   }
...
int GetStrategyEntryHour(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(cfg.special_rule == RULE_NONE && InpTestMode && InpUseTestTimes)
   {
...
bool IsOvernightExit(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(exit_hour == entry_hour && exit_minute < entry_minute)
   {
      return true;
   }
...
bool IsExitTime(StrategyConfig &cfg, datetime jst_time)
{
   ...
   if(overnight_exit)
   {
      ...
      if(dt.hour == entry_hour && dt.min >= entry_minute)
      {
         return false;
      }
...
{
   // Step 2F existing 13
   SetStrategy(0, InpEnable_17_EA_1B, "17_EA_1B_Wed_Short", "17_EA_1B", "EURAUD", 17001, DIR_SHORT, 9, 59, 20, 58, 70.0, 175.0, "17_EA_1B_step3", RULE_NONE);
   SetWeekdays(strategies[0], false, false, false, true, false, false, false);

   SetStrategy(1, InpEnable_18_EA_2, "18_EA_2_MonWed_Short", "18_EA_2", "EURAUD", 18002, DIR_SHORT, 9, 59, 5, 26, 90.0, 180.0, "18_EA_2_step3", RULE_NONE);
   SetWeekdays(strategies[1], false, true, true, true, false, false, false);

   SetStrategy(2, InpEnable_19_EA_3, "19_EA_3_WedThu_Long", "19_EA_3", "EURAUD", 19003, DIR_LONG, 20, 56, 10, 0, 90.0, 0.0, "19_EA_3_step3", RULE_NONE);
   SetWeekdays(strategies[2], false, false, false, true, true, false, false);

   SetStrategy(3, InpEnable_21_GA_B3, "21_GA_B_3", "21_GA_B3", "GBPAUD", 21003, DIR_LONG, 21, 2, 10, 0, 220.0, 100.0, "21_GA_B_3_step3", RULE_NONE);
   SetWeekdays(strategies[3], false, true, false, false, false, false, false);

   SetStrategy(4, InpEnable_22_GA_C2, "22_GA_C_2", "22_GA_C2", "GBPAUD", 22002, DIR_LONG, 16, 56, 1, 15, 70.0, 80.0, "22_GA_C_2_step3", RULE_NONE);
   SetWeekdays(strategies[4], false, false, false, false, true, false, false);

   SetStrategy(5, InpEnable_23_GA_F2, "23_GA_F_2", "23_GA_F2", "GBPAUD", 23002, DIR_SHORT, 19, 42, 22, 45, 90.0, 200.0, "23_GA_F_2_step3", RULE_NONE);
   SetWeekdays(strategies[5], false, false, false, false, false, true, false);

   SetStrategy(6, InpEnable_24_GA_D1, "24_GA_D_1", "24_GA_D1", "GBPAUD", 24001, DIR_LONG, 22, 44, 3, 8, 90.0, 200.0, "24_GA_D_1_step3", RULE_NONE);
   SetWeekdays(strategies[6], false, false, false, false, false, true, false);

   SetStrategy(7, InpEnable_5_GJ_Log2, "5_GJ_Port_Log2", "5_GJ_Log2", "GBPJPY", 50002, DIR_SHORT, 9, 55, 23, 55, 90.0, 0.0, "5_GJ_Port_Log2_step3", RULE_NONE);
   SetWeekdays(strategies[7], false, false, true, false, true, true, false);

   SetStrategy(8, InpEnable_12_UJ_Short_Core, "12_UJ_Short_Core", "12_UJ_Short_Core", "USDJPY", 12001, DIR_SHORT, 8, 4, 14, 56, 50.0, 0.0, "12_UJ_Short_Core_step3", RULE_UJ_SHORT_CORE);
   SetWeekdays(strategies[8], false, true, true, true, true, true, false);

   SetStrategy(9, InpEnable_13_UJ_Fix_MidWeek, "13_UJ_Fix_MidWeek", "13_UJ_Fix_MidWeek", "USDJPY", 13001, DIR_LONG, 18, 4, 22, 3, 95.0, 95.0, "13_UJ_Fix_MidWeek_step3", RULE_UJ_FIX_MIDWEEK);
   SetWeekdays(strategies[9], false, false, false, true, true, false, false);

   SetStrategy(10, InpEnable_14_UJ_Sat_3rd, "14_UJ_Sat_3rd", "14_UJ_Sat_3rd", "USDJPY", 14001, DIR_SHORT, 20, 1, 3, 8, 45.0, 70.0, "14_UJ_Sat_3rd_step3", RULE_UJ_SAT_3RD);
   SetWeekdays(strategies[10], false, true, true, true, true, true, false);

   SetStrategy(11, InpEnable_15_UJ_Sat_Aug, "15_UJ_Sat_Aug", "15_UJ_Sat_Aug", "USDJPY", 15001, DIR_SHORT, 19, 0, 23, 30, 20.0, 35.0, "15_UJ_Sat_Aug_step3", RULE_UJ_SAT_AUG);
   SetWeekdays(strategies[11], false, true, true, true, true, true, false);

   SetStrategy(12, InpEnable_16_UJ_T10A, "16_UJ_T10A", "16_UJ_T10A", "USDJPY", 16001, DIR_LONG, 2, 58, 9, 50, 45.0, 110.0, "16_UJ_T10A_step3", RULE_UJ_T10A);
   SetWeekdays(strategies[12], false, true, true, true, true, true, false);

   // Step 2H added 10
   SetStrategy(13, InpEnable_1_EJ_Log1, "1_EJ_Log1", "1_EJ_Log1", "EURJPY", 10001, DIR_LONG, 13, 55, 4, 55, 70.0, 250.0, "1_EJ_Log1_step3", RULE_NONE);
   SetWeekdays(strategies[13], false, true, false, true, false, false, false);

   SetStrategy(14, InpEnable_2_EJ_NightBlitz_20, "2_EJ_NightBlitz_20", "2_EJ_NightBlitz_20", "EURJPY", 20001, DIR_LONG, 20, 56, 4, 45, 45.0, 70.0, "2_EJ_NightBlitz_20_step3", RULE_NONE);
   SetWeekdays(strategies[14], false, true, false, true, false, false, false);

   SetStrategy(15, InpEnable_3_EJ_NightBlitz_21, "3_EJ_NightBlitz_21", "3_EJ_NightBlitz_21", "EURJPY", 30001, DIR_LONG, 21, 56, 5, 27, 75.0, 70.0, "3_EJ_NightBlitz_21_step3", RULE_NONE);
   SetWeekdays(strategies[15], false, true, false, true, false, false, false);

   SetStrategy(16, InpEnable_4_GJ_Port_Log1, "4_GJ_Port_Log1", "4_GJ_Port_Log1", "GBPJPY", 40001, DIR_LONG, 0, 0, 8, 55, 130.0, 90.0, "4_GJ_Port_Log1_step3", RULE_NONE);
   SetWeekdays(strategies[16], false, false, true, true, false, false, false);

   SetStrategy(17, InpEnable_6_GJ_Old_Mon, "6_GJ_Old_Mon", "6_GJ_Old_Mon", "GBPJPY", 60001, DIR_LONG, 15, 45, 22, 50, 50.0, 210.0, "6_GJ_Old_Mon_step3", RULE_NONE);
   SetWeekdays(strategies[17], false, true, false, false, false, false, false);

   SetStrategy(18, InpEnable_7_GJ_Mon_Blitz, "7_GJ_Mon_Blitz", "7_GJ_Mon_Blitz", "GBPJPY", 70001, DIR_LONG, 18, 2, 23, 2, 130.0, 250.0, "7_GJ_Mon_Blitz_step3", RULE_NONE);
   SetWeekdays(strategies[18], false, true, false, false, false, false, false);

   SetStrategy(19, InpEnable_8_AJ_Core1, "8_AJ_Core1", "8_AJ_Core1", "AUDJPY", 80001, DIR_LONG, 8, 1, 22, 46, 70.0, 110.0, "8_AJ_Core1_step3", RULE_NONE);
   SetWeekdays(strategies[19], false, true, false, false, false, false, false);

   SetStrategy(20, InpEnable_10_AJ_SatA, "10_AJ_SatA", "10_AJ_SatA", "AUDJPY", 10002, DIR_SHORT, 10, 58, 13, 51, 50.0, 25.0, "10_AJ_SatA_step3", RULE_NONE);
   SetWeekdays(strategies[20], false, false, false, false, false, true, false);

   SetStrategy(21, InpEnable_11_AJ_SatB, "11_AJ_SatB", "11_AJ_SatB", "AUDJPY", 11001, DIR_SHORT, 18, 57, 1, 43, 55.0, 95.0, "11_AJ_SatB_step3", RULE_NONE);
   SetWeekdays(strategies[21], false, false, false, false, false, true, false);

   SetStrategy(22, InpEnable_20_EA_1A, "20_EA_1A_MonTue_Short", "20_EA_1A", "EURAUD", 20002, DIR_SHORT, 10, 1, 16, 0, 50.0, 125.0, "20_EA_1A_step3", RULE_NONE);
   SetWeekdays(strategies[22], false, true, true, false, false, false, false);

   // Step 2I China 4
   SetStrategy(23, InpEnable_25_AU_China_Demand, "25_AU_China_Demand", "25_AU_China_Demand", "AUDUSD", 25001, DIR_LONG, 10, 0, 15, 50, 40.0, 40.0, "25_AU_China_Demand_step3", RULE_CHINA_AU_DEMAND);
   SetWeekdays(strategies[23], false, true, true, true, true, true, false);

   SetStrategy(24, InpEnable_26_AJ_China_Demand, "26_AJ_China_Demand", "26_AJ_China_Demand", "AUDJPY", 26001, DIR_LONG, 10, 0, 15, 50, 45.0, 80.0, "26_AJ_China_Demand_step3", RULE_CHINA_9_15);
   SetWeekdays(strategies[24], false, true, true, true, true, true, false);

   SetStrategy(25, InpEnable_27_EA_China_Demand, "27_EA_China_Demand", "27_EA_China_Demand", "EURAUD", 27001, DIR_SHORT, 10, 0, 15, 50, 60.0, 60.0, "27_EA_China_Demand_step3", RULE_CHINA_9_15);
   SetWeekdays(strategies[25], false, true, true, true, true, true, false);

   SetStrategy(26, InpEnable_28_GA_China_Demand, "28_GA_China_Demand", "28_GA_China_Demand", "GBPAUD", 28001, DIR_SHORT, 10, 0, 16, 10, 75.0, 70.0, "28_GA_China_Demand_step3", RULE_CHINA_9_15);
   SetWeekdays(strategies[26], false, true, true, true, true, true, false);

   // Step 2J AJ Core2
   SetStrategy(27, InpEnable_9_AJ_Core2, "9_AJ_Core2", "9_AJ_Core2", "AUDJPY", 90001, DIR_SHORT, 17, 14, 1, 14, 30.0, 80.0, "9_AJ_Core2_step3", RULE_AJ_CORE2);
...
void TryEntry(StrategyConfig &cfg, datetime jst_time)
{
   ...
   {
      SendBuyOrder(cfg, jst_time);
      return;
   }

   if(cfg.direction == DIR_SHORT)
   {
      SendSellOrder(cfg, jst_time);
      return;
   }
...
{
   PrintDebug("EA", "Step 3 Clean initialized.");
   PrintDebug("EA", "Config-managed 28 strategy EA.");
...
void OnDeinit(const int reason)
{
   ...
   PrintDebug("EA", "Step 3 Clean deinitialized. Reason=" + IntegerToString(reason));
}
