"""Preregistered descriptive volatility diagnosis. No trade generation or filtering."""
from pathlib import Path
import argparse, hashlib, json, math
import numpy as np
import pandas as pd

PLAN_COMMIT = 'cb36e1c7d5403c9be40480cdcfbaf221b32fda00'
...
FROZEN = json.loads(Path(__file__).with_name('volatility_phase1_frozen_inputs.json').read_text())
STRATEGIES = FROZEN['STRATEGIES']
PAIR_SYMBOL = {v:k for k,v in FROZEN['SYMBOL_TO_PAIR'].items()}
REGIMES = ['LOW','NORMAL','HIGH']
INS = 'INSUFFICIENT_VOL_HISTORY'
...
def features(d):
    ...
    d['LogReturn']=np.log(d.Close/d.Close.shift())
    d['RV20']=d.LogReturn.rolling(20,min_periods=20).std(ddof=1)*np.sqrt(252)
...
def tables_for(t,method,period,cis):
    ...
    for s in STRATEGIES:
        ...
        for reg in REGIMES: cells.append(dict(Period=period,Method=method,StrategyNo=no,Strategy=s['Strategy'],Symbol=PAIR_SYMBOL[s['Pair']],Direction='Long' if s['Long'] else 'Short',Regime=reg,**m[reg],HighMinusLowAvgR=diff,MaxMinusMinAvgR=rng,Eligible=eligible,**ci,ClearDependence=eligible and clear(ci) if ci else False))
    groups=[]
...
def manual_audit(bars,d,own):
    # Preregistered dates chosen by data availability, never by R or effect size.
    indices=[]
    ...
    for i in sorted(set(indices)):
        ...
        for j in range(i-19,i+1):
            z=d.iloc[j];prev=d.iloc[j-1].Close if j else z.Close
            tr.append(max(z.High-z.Low,abs(z.High-prev),abs(z.Low-prev)))
        atr=math.fsum(tr)/20
        returns=[math.log(d.iloc[j].Close/d.iloc[j-1].Close) for j in range(i-19,i+1)]
        mean=math.fsum(returns)/20;rv=math.sqrt(math.fsum((v-mean)**2 for v in returns)/19)*math.sqrt(252)
        assert math.isclose(row.ATR20,atr,abs_tol=1e-12) and math.isclose(row.RV20,rv,abs_tol=1e-10)
        for method,col in [('primary','ATR20'),('robustness','RV20')]:
            ref=d[col].iloc[i-252:i].to_numpy();value=row[col]
            expected=INS
            if len(ref)==252 and np.isfinite(ref).all():
                rank=(sum(v<value for v in ref)+.5*sum(v==value for v in ref))/252
                expected=REGIMES[0] if rank<1/3 else REGIMES[1] if rank<2/3 else REGIMES[2]
            assert row[method]==expected
        representative=own[own.DailyDate==row.DailyDate].sort_values('EntryTime')
        out.append(dict(DailyDate=row.DailyDate,DailyIndex=i,LastM1=row.LastM1,AvailableAt=row.AvailableAt,Open=row.Open,High=row.High,Low=row.Low,Close=row.Close,M1Count=len(b),ATR20=row.ATR20,IndependentATR20=atr,RV20=row.RV20,IndependentRV20=rv,ReferenceStart=row.ReferenceStart,ReferenceEnd=row.ReferenceEnd,primary=row.primary,robustness=row.robustness,RepresentativeEntry=representative.EntryTime.iloc[0] if len(representative) else '',Status='PASS'))
    return out
...
def run(baseline,m1_root,output_dir,implementation_sha):
    ...
    for symbol,names in FROZEN['MANIFEST_NAMES'].items():
        ...
        for name in names:
            ...
            paths.append(hits[0])
        bars,m=load_m1(paths);manifest.extend(dict(Symbol=symbol,**r) for r in m)
        d=features(make_daily(bars));own=assign(t[t.Symbol==symbol],d)
    ...
    cols=['TradeID','StrategyNo','Strategy','Symbol','Direction','EntryTime','R','DailyDate','DailyIndex','LastM1','AvailableAt','ATR20','RV20','ReferenceStart','ReferenceEnd','primaryPercentile','robustnessPercentile','primary','robustness']
    tables['assignment_audit_light']=a[a.TradeID.isin(audit_ids)][cols]
    from verify_volatility_phase1 import verify_tables
    checks=verify_tables(a,tables);tables['verification']=pd.DataFrame(checks)
    hashes={}
    for name,frame in tables.items():
        p=out/f'volatility_phase1_{name}.csv';frame.to_csv(p,index=False);hashes[p.name]=sha(p)
    record=dict(Status='COMPLETED',Branch=BRANCH,PlanCommit=PLAN_COMMIT,ImplementationCommit=implementation_sha,RunJST=pd.Timestamp.now(tz='Asia/Tokyo').isoformat(),BaselineSHA256=sha(baseline),BaselineTrades=len(t),StrategyCount=28,BaselineRecalculated=False,EntryTradesRemoved=0,LiveChanged=False,FreshHoldout=False,ATRSpec='D1 TR SMA20 (existing implementation priority)',RVSpec='20 log returns sample std ddof1 sqrt252',PercentileSpec='midrank previous252 excluding evaluated day',DailySpec='JST actual bars; next midnight availability; Saturday included',Bootstrap='5000 calendar-week clusters seed20260913,95%linear CI; unadjusted',InputFiles=len(manifest),IndependentVerification='PASS',PrimaryAssigned=int(a.primary.ne(INS).sum()),PrimaryInsufficient=int(a.primary.eq(INS).sum()),RobustnessAssigned=int(a.robustness.ne(INS).sum()),RobustnessInsufficient=int(a.robustness.eq(INS).sum()),Python=__import__('sys').version.split()[0],Pandas=pd.__version__,Numpy=np.__version__,OutputHashes=json.dumps(hashes,sort_keys=True))
    pd.DataFrame([record]).to_csv(out/'volatility_phase1_run_record.csv',index=False)
    for name in ['combined_decision','decision','strategy_primary','strategy_robustness']:
