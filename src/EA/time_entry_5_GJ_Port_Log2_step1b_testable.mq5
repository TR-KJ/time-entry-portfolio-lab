...
//| Step 1B: 5_GJ_Port_Log2 single strategy EA - testable version    |
//+------------------------------------------------------------------+
#property strict
...
// Test mode
input bool   InpTestModeIgnoreWeekday = true;
...
void PrintDebug(string message)
{
   if(InpPrintDebug)
   {
      Print("[5_GJ_Port_Log2 Step1B Testable] ", message);
   }
}
...
{
   string name = "TE_5_GJ_PORT_LOG2_STEP1B_TESTABLE_" + _Symbol + "_" + IntegerToString(InpMagicNumber) + "_" + IntegerToString(date_key);
...
void TryEntry()
{
   ...
   string comment = "5_GJ_Port_Log2_step1b_testable";
...
int OnInit()
{
   ...
   PrintDebug("TP=None");
   PrintDebug("TestModeIgnoreWeekday=" + (InpTestModeIgnoreWeekday ? "true" : "false"));
