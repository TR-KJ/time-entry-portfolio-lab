# Strategy Selection / Portfolio Pruning（戦略選択／ポートフォリオ整理）検証計画

状態: 固定済み — 実装・集計・OOS開封前
...
戦略数を減らすこと自体、見た目を単純化すること、PFまたはDDだけを改善することは目的としない。利益額の増加を最優先とし、戦略除外が再現性のある改善を示さない場合は28戦略のベースラインを維持する。

## 2. 固定ベースラインと母集団

Daily Stop検証で固定したベースライン・トレードログだけを母集団として使用する。
...
- Daily Stop: なし
- ATR: OFF
- イベントフィルター: Candidate C
- UJ12前倒しゴトウビ修正: 反映済み
- EJ1イベント重複修正: 反映済み

ベースライン・トレードログは再計算しない。M1データからのバックテスト再実行、約定結果の再生成、決済条件の変更は行わず、固定ログから対象戦略の行を除外するシミュレーションだけを行う。ベースラインと各候補の差は、同一母集団に対する除外だけで生じなければならない。
...
停止候補の判定ルール、順位、候補ポートフォリオおよびしきい値はISだけで決定・固定する。OOS1を確認した後に、候補の追加・削除・入替え、しきい値変更、グループ追加、追加探索をしてはならない。OOS2は最終ホールドアウトとし、すべての候補と判定ルールを固定した後、最後に一度だけ確認する。
...
### 4.2 Leave-One-Out（1戦略ずつ除外）

次に、ベースラインから1戦略だけを除外した28通りをISで集計する。各結果は同じベースラインと比較する。
...
各戦略の所属は、固定ベースラインに対応する既存の戦略定義・設定値・タグから機械的に決める。重複所属を許す。成績を見て所属を変更してはならず、初回集計前に戦略名と所属を`results/strategy_selection/strategy_selection_group_membership.csv`へ固定する。上記以外のグループ、後付けの名称、任意の通貨ペア集合は今回検証しない。

## 6. 指標

最優先の主指標は`Total R`とする。

単体健康診断、ベースライン、グループおよび候補ポートフォリオでは、少なくとも次を併記する。

- Trades（トレード数）
- Total R（合計R）
- PF
- Win Rate（勝率）
- Avg R / trade（1トレード当たり平均R）
- Avg Win R（平均勝ちR）
- Avg Loss R（平均負けR）
- Max DD R（最大ドローダウンR）
- Worst Day R（JST日単位の最悪日R）
- Worst Week R（JST、月曜開始の最悪週R）
- 年別Total R
- positive years / negative years（プラス年数／マイナス年数。Total Rが0の年はneutral＝中立）
- max losing streak（最大連敗数）
- exit reason率（TP / SL / time / その他。固定ログ上の決済理由に従う）

Leave-One-Outでは、ベースライン比として少なくとも次を表示する。
...
Deltaは原則`候補 - ベースライン`とする。Max DD Rは損失深度を正の絶対値で表し、Delta Max DD Rが負なら改善である。Worst Day RとWorst Week Rは負値で表し、Deltaが正なら改善である。符号規約は全CSVで統一する。
...
「安定して負の限界貢献」と呼ぶには、上記に加えてISの7暦年のうちnegative years（マイナス年数）がpositive years（プラス年数）を上回ることを必要とする。年ごとの符号が混在する戦略を、直近だけ・特定年だけを理由に除外しない。
...
IS集計後、OOS開封前に次の候補ポートフォリオだけを固定する。該当候補が存在しない枠は作らない。
...
最大でもベースラインを含む5候補とする。候補ID、除外戦略、選定根拠、IS値を`results/strategy_selection/strategy_selection_candidates_frozen.csv`に記録し、そのファイルをOOS実行前にコミットする。

候補枠を埋めるために条件を緩めない。ISで利益改善候補がなければベースラインだけで終了し、「除外候補なし」と結論する。
...
固定した全候補ポートフォリオを同じ条件でOOS1に一度だけ適用し、その後OOS2を一度だけ確認する。OOS結果を見て候補ポートフォリオを作り直さない。

資金シミュレーションへ進めるのは、OOS1およびOOS2の確認後に残った最終候補だけとする。方式は次の2つに限定する。

- Weekly Fixed Risk（週次固定リスク）
- Weekly Compound（週次複利）

最終判断では、ベースラインに対する利益額の増加を第一条件とする。Max DD、Worst Day、Worst Week等の改善は副次評価である。利益額がベースラインより減る候補は、DDが改善しても採用しない。結果が小さい、期間間で不安定、またはOOSで再現しない場合はベースラインを維持する。

## 11. GBPAUD 2019年の既知欠損

GBPAUD 2019年の既知欠損はDaily Stop検証と同じ扱いとする。

- 欠損を補完しない
- 新しい除外日を設定しない
- 固定ベースライン・トレードログをそのまま使用する
...
研究完了時は、各候補および検証全体について`ADOPT`（採用）または`REJECT`（不採用）を明示し、ベースラインを維持する場合も明記する。

戦略除外をEAへ反映するかは、IS・OOS1・OOS2・資金シミュレーションの完了後に別途判断する。本計画書の作成および今後の研究コード実装は、EA、VPS、SET、live運用コードまたは稼働設定を変更する許可を意味しない。
