...
def verify(baseline, output_dir):
    ...
    out=Path(output_dir);prefix='edge_decay_phase2_20_'
    ...
    for label,(start,end) in periods.items():
        ...
        expected=[]
        for frame in (sub,sub[sub.Strategy!='20_EA_1A_MonTue_Short']):
        ...
        assert delta['TotalR']==-sum(sub.loc[sub.Strategy=='20_EA_1A_MonTue_Short','exact'],Decimal(0))
        if label.isdigit():annual[int(label)]=delta['TotalR']
        for name,exp in zip(('E0_BASELINE','E1_MINUS_20','DELTA_E1_MINUS_E0'),expected):
