...
//| Step 5.2.1: Event filter with robust log suppression           |
//+------------------------------------------------------------------+
#property strict
...
void PrintDebug(string strategy, string message)
{
   if(InpPrintDebug)
   {
      Print("[Step5 Event ", strategy, "] ", message);
   }
}
...
void PrintAtrDecisionLog(StrategyConfig &cfg, datetime jst_time, string message)
{
   ...
   {
      Print("[Step5 Event ", cfg.strategy_name, "] ", message);
   }
}
...
bool ShouldPrintEventDecisionLog(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   ...
   string gv_name = EventLogGlobalVariableName(cfg, jst_time, event_name);

   if(WasEventLogPrintedRuntime(gv_name))
   {
      return false;
   }
   ...
   if(GlobalVariableCheck(gv_name))
   {
      MarkEventLogPrintedRuntime(gv_name);
      return false;
   }
...
void PrintEventDecisionLog(StrategyConfig &cfg, datetime jst_time, string event_name)
{
   ...
   {
      int key = DateKey(jst_time);
      Print("[Step5 Event ", cfg.strategy_name, "] EVENT REJECT. Symbol=", cfg.symbol, ", Event=", event_name, ", Date=", IntegerToString(key));
   }
}
...
bool IsEvents7AJDate(int date_key, string &event_name)
{
   if(IsEvents4Date(date_key, event_name))
   {
      return true;
...
bool PassPythonCalendarEventFilter(StrategyConfig &cfg, datetime jst_time)
{
   ...
   {
      if(IsEvents7AJDate(date_key, event_name))
      {
         PrintEventDecisionLog(cfg, jst_time, event_name);
         return false;
      }
   ...
   // EA / GA / China rules were not included in the provided Python calendar generator.
   // Initial Step 5 follows the provided Python logic and leaves them unaffected.
   return true;
}
...
{
   PrintDebug("EA", "Step 5.2.1 Event filter log-suppressed initialized.");
   PrintDebug("EA", "Config-managed 28 strategy EA with ATR P70 and Python-calendar event filter.");
