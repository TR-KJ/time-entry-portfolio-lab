...
//| Step 2B: Config-managed 2 strategy EA                            |
//|                                                                  |
//| Strategies:                                                      |
...
//+------------------------------------------------------------------+
//| Test time override inputs                                        |
//| If InpUseTestTimes = true, both strategies use these times.       |
//+------------------------------------------------------------------+
...
{
   string name = "TE_STEP2B_" + cfg.strategy_code + "_" + cfg.symbol + "_" + IntegerToString(cfg.magic) + "_" + IntegerToString(date_key);
...
void SetupStrategies()
{
   ...
   // 22_GA_C_2
   // ---------------------------------------------------------------
   strategies[0].enabled = InpEnable_GA_C2;
   strategies[0].strategy_name = "22_GA_C_2";
   strategies[0].strategy_code = "22_GA_C2";
   strategies[0].symbol = "GBPAUD";
   strategies[0].magic = 22002;
   strategies[0].direction = DIR_LONG;

   strategies[0].trade_sunday = false;
   strategies[0].trade_monday = false;
   strategies[0].trade_tuesday = false;
   strategies[0].trade_wednesday = false;
   strategies[0].trade_thursday = true;
   strategies[0].trade_friday = false;
   strategies[0].trade_saturday = false;

   strategies[0].entry_hour = 16;
   strategies[0].entry_minute = 56;

   strategies[0].exit_mode = EXIT_SPECIFIC_WEEKDAY;
   strategies[0].exit_weekday = 5;
   strategies[0].exit_hour = 1;
   strategies[0].exit_minute = 15;

   strategies[0].sl_pips = 70.0;
   strategies[0].tp_pips = 80.0;

   strategies[0].one_entry_per_day = true;
   strategies[0].comment = "22_GA_C_2_step2b";

   // ---------------------------------------------------------------
   // 5_GJ_Port_Log2
   // ---------------------------------------------------------------
   strategies[1].enabled = InpEnable_GJ_Log2;
   strategies[1].strategy_name = "5_GJ_Port_Log2";
   strategies[1].strategy_code = "5_GJ_Log2";
   strategies[1].symbol = "GBPJPY";
   strategies[1].magic = 50002;
   strategies[1].direction = DIR_SHORT;

   strategies[1].trade_sunday = false;
   strategies[1].trade_monday = false;
   strategies[1].trade_tuesday = true;
   strategies[1].trade_wednesday = false;
   strategies[1].trade_thursday = true;
   strategies[1].trade_friday = true;
   strategies[1].trade_saturday = false;

   strategies[1].entry_hour = 9;
   strategies[1].entry_minute = 55;

   strategies[1].exit_mode = EXIT_SAME_DAY;
   strategies[1].exit_weekday = -1;
   strategies[1].exit_hour = 23;
   strategies[1].exit_minute = 55;

   strategies[1].sl_pips = 90.0;
   strategies[1].tp_pips = 0.0;

   strategies[1].one_entry_per_day = true;
   strategies[1].comment = "5_GJ_Port_Log2_step2b";
}
...
int OnInit()
{
   ...
   PrintDebug("EA", "Step 2B initialized.");
   PrintDebug("EA", "Config-managed 2 strategies.");
...
void OnDeinit(const int reason)
{
   ...
   PrintDebug("EA", "Step 2B deinitialized. Reason=" + IntegerToString(reason));
}
