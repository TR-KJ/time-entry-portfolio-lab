...
//| Step 2E.2 FIX1: UJ 5 logic test EA                               |
//|                                                                  |
//| Fix: Overnight exit handling for 14_UJ_Sat_3rd                   |
//|                                                                  |
//| Strategies:                                                      |
...
void PrintDebug(string strategy, string message)
{
   if(InpPrintDebug)
   {
      Print("[Step2E.2 FIX1 ", strategy, "] ", message);
   }
}
...
{
   string name = "TE_STEP2E2_FIX1_" + cfg.strategy_code + "_" + cfg.symbol + "_" + IntegerToString(cfg.magic) + "_" + IntegerToString(date_key);
...
bool IsOvernightExit(StrategyConfig &cfg)
{
   if(cfg.exit_hour < cfg.entry_hour)
   {
      return true;
   }

   if(cfg.exit_hour == cfg.entry_hour && cfg.exit_minute < cfg.entry_minute)
   {
      return true;
   }
...
bool IsExitTime(StrategyConfig &cfg, datetime jst_time)
{
   ...
   bool overnight_exit = IsOvernightExit(cfg);

   if(overnight_exit)
   {
      // For overnight exits, do not close on the entry-day evening.
      // Example: Entry 20:01, Exit next day 03:08.
      if(dt.hour > cfg.entry_hour)
      {
         return false;
      }

      if(dt.hour == cfg.entry_hour && dt.min >= cfg.entry_minute)
      {
         return false;
      }
...
void SetupStrategies()
{
   ...
   SetStrategy(
      ...
      95.0,
      95.0,
      "13_UJ_Fix_MidWeek_step2e2_fix1",
      RULE_UJ_FIX_MIDWEEK
   );
...
int OnInit()
{
   ...
   PrintDebug("EA", "Step 2E.2 FIX1 initialized.");
   PrintDebug("EA", "UJ 5 logic test EA.");
   PrintDebug("EA", "Overnight exit fix enabled.");
...
void OnDeinit(const int reason)
{
   ...
   PrintDebug("EA", "Step 2E.2 FIX1 deinitialized. Reason=" + IntegerToString(reason));
}
