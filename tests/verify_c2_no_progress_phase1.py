"""Independent arithmetic and representative M1 verification; no C2 replay imports."""
import argparse
import csv
import hashlib
import math
...
def main():
    ...
    for p in read('period_summary'):
        ...
        check('totals_'+p['Variant']+'_'+p['Period'],len(rr)==int(p['Trades']) and len(trigger)==int(p['TriggerCount']) and
        ...
        check('bootstrap_'+p['Variant']+'_'+p['Period'],eq(bounds[0],p['CILower']) and eq(bounds[1],p['CIUpper']))
        for tag,value,timekey in [('R0','R0R','R0CloseTime'),('NP','NPR','NPCloseTime')]:
            ...
            check('safety_metrics_'+tag+'_'+p['Variant']+'_'+p['Period'],eq(min(day.values()),p[tag+'WorstDayR']) and eq(min(wk.values()),p[tag+'WorstWeekR']) and eq(dd,p[tag+'MaxDDR']))
    ...
    for symbol,pair in SYMBOL_TO_PAIR.items():
        ...
        for r in selected.values():
            ...
            prior=data.loc[(data.index>=e)&(data.index<cp)]
            if datetime.fromisoformat(r['R0CloseTime'])<cp:
                ...
                assessment='PROGRESS_REACHED' if reached else ('NO_PROGRESS_EXIT' if triggered else 'CLOSED_BEFORE_EXECUTION')
