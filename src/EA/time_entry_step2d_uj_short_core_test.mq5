...
//| Step 2D: 12_UJ_Short_Core single strategy test EA                |
//+------------------------------------------------------------------+
#property strict
...
void PrintDebug(string message)
{
   if(InpPrintDebug)
   {
      Print("[Step2D 12_UJ_Short_Core] ", message);
   }
}
...
{
   string name = "TE_STEP2D_12_UJ_SHORT_CORE_" + _Symbol + "_" + IntegerToString(InpMagicNumber) + "_" + IntegerToString(date_key);
...
{
   int next_day = dt.day + 1;
   int next_month = dt.mon;
...
bool IsGotoDay(int day)
{
   ...
   // No forward-shift goto day handling in Step 2D.
   if(day == 20)
   {
      return true;
   }
...
void TryEntry()
{
   ...
   string comment = "12_UJ_Short_Core_step2d_" + mode_text;
   ...
   if(result)
   {
      ...
      if(tp > 0)
      {
         PrintDebug("SELL entry success. Mode=" + mode_text + ", Lot=" + DoubleToString(lot, 2) + ", Bid=" + DoubleToString(bid, digits) + ", SL=" + DoubleToString(sl, digits) + ", TP=" + DoubleToString(tp, digits));
      }
      else
      {
         PrintDebug("SELL entry success. Mode=" + mode_text + ", Lot=" + DoubleToString(lot, 2) + ", Bid=" + DoubleToString(bid, digits) + ", SL=" + DoubleToString(sl, digits) + ", TP=None");
      }
   }
   else
   {
