...
EVENTS_5 = EVENTS_4.union(set(ECB_DATES)) # UJ/EJ用
...
def load_data(pair_name):
    ...
    if not files: return None, None, None, None, None, None
...
# ===== 4. 実行部 =====
print("データを読み込み、全16ロジックのバックテストを一斉実行中...")
...
run_strategy('UJ', '12_UJ_Short_Core', t_uj, o_uj, h_uj, l_uj, idx_uj, dates_uj, False, 0.005, f_uj, [], 9, 55, 14, 56, 0, 20, 50, True, '20_to_end')
...
# ===== 5. ポートフォリオ集計 =====
if all_trades:
    df_res = pd.DataFrame(all_trades).sort_values('CloseTime').reset_index(drop=True)
    df_res['Cum_Pips'] = df_res['Pips'].cumsum()
    df_res['Max_Cum_Pips'] = df_res['Cum_Pips'].cummax()
    df_res['Drawdown'] = df_res['Max_Cum_Pips'] - df_res['Cum_Pips']
    
    total_pips = df_res['Cum_Pips'].iloc[-1]
    max_dd = df_res['Drawdown'].max()
    romd = total_pips / max_dd if max_dd > 0 else 0
    total_trades = len(df_res)
    win_rate = len(df_res[df_res['Pips'] > 0]) / total_trades * 100
    
    print("\n" + "="*50)
    print("🏆 ポートフォリオ統合テスト結果 (全16ロジック)")
    print("="*50)
    print(f"総トレード数: {total_trades} 回 (勝率 {win_rate:.1f}%)")
    print(f"合計獲得Pips: {total_pips:.1f} pips")
    print(f"合成最大ドローダウン: {max_dd:.1f} pips")
    print(f"総合 RoMD: {romd:.2f}")
    
    # 年別集計
    df_res['Year'] = pd.to_datetime(df_res['CloseTime']).dt.year
    yearly = df_res.groupby('Year')['Pips'].sum().round(1)
    print("\n--- 年別獲得Pips ---")
    print(yearly.to_string())
    
    df_res.to_csv('/content/Portfolio_Integration_Results.csv', index=False)
    print("\n👉 詳細なトレード履歴を '/content/Portfolio_Integration_Results.csv' に保存しました。")
else:
    print("データが見つからないか、トレードが発生しませんでした。")
