...
def verify(baseline, output_dir):
    ...
    out=Path(output_dir);prefix='edge_decay_phase2_23_'
    ...
    for label,(start,end) in periods.items():
        ...
        expected=[]
        for frame in (sub,sub[sub.Strategy!='23_GA_F_2']):
        ...
        assert delta['TotalR']==-sum(sub.loc[sub.Strategy=='23_GA_F_2','exact'],Decimal(0))
        if label.isdigit():annual[int(label)]=delta['TotalR']
        for name,exp in zip(('E0_BASELINE','E1_MINUS_23','DELTA_E1_MINUS_E0'),expected):
